...
## <img src="./docs/assets/vita-icon-sm.png" alt="" height="30" style="vertical-align: text-bottom; margin-right: 6px;" /> 项目简介

治疗性抗体发现并非线性流程，而是由免疫应答、候选筛选、序列获取、重组表达与多维表征等多个阶段相互衔接构成的实验体系。各阶段之间既有先后依赖，也存在并行路径与跨环节共享支撑，难以用单一顺序概括全部工作。

典型研发路径以动物免疫激发体液应答为起点，通过血清效价及 ELISA、流式细胞术等检测评估是否进入下游；随后经 **单 B 细胞筛选**、**噬菌体展示** 等策略并行推进，结合文库构建与 Sanger / NGS 测序获得候选序列；再完成质粒与细胞制备、转染、表达与纯化（含 LNP 等递送相关准备），并开展结合、亲和力、功能及理化 / 成药性评价。行业高通量单细胞与展示筛选流程，普遍围绕「应答 → 筛选 → 序列 → 表达 → 表征」这一主线组织；分子与细胞制备、实验室自动化等能力则贯穿多个阶段按需调用，而非仅出现在主链末端。

**Antibody Vita**（仓库名 Antibody Forge）是面向百奥赛图免疫研发场景的抗体发现全流程协作平台。系统将上述主线及共享支撑统一纳入 Web 平台：按实际实验路径组织模块、数据与工单，以 `experiment_id` 等业务主键关联跨环节记录，遵循单一数据源原则，避免同一事实多处维护。

对需设备执行的环节（如效价流式检测），平台以「工单创建 → 任务下发 → 结果回传」对接自动化模组，贯通发现全链路协同，而非将既有分散流程简单迁移至线上。主链与各支撑层的关系见下方流程图；各模块建设进度见 [docs/README.md](./docs/README.md)。
