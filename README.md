...
<h2 style="display: flex; align-items: center; gap: 10px;">
  <img src="./docs/assets/vita-icon-sm.png" alt="" height="38" width="38" />
  <span>项目简介</span>
</h2>

治疗性抗体发现并非线性流程，而是由免疫应答、候选筛选、序列获取、重组表达与多维表征等多个阶段相互衔接构成的实验体系。

典型路径以动物免疫激发体液应答为起点，通过血清效价及 ELISA、流式细胞术等检测评估是否进入下游。进而经 **单 B 细胞筛选**、**噬菌体展示** 等策略并行推进，结合文库构建与 Sanger / NGS 测序获得候选序列。

获得序列后，需完成质粒与细胞制备、转染、表达与纯化（含 LNP 等递送相关准备），并开展结合、亲和力、功能及理化 / 成药性评价。行业高通量流程普遍围绕「应答 → 筛选 → 序列 → 表达 → 表征」这一主线组织。

**Antibody Vita**（仓库名 Antibody Forge）面向百奥赛图免疫研发场景，是抗体发现全流程协作平台。系统按实际实验路径组织数据与工单，遵循单一数据源原则；对需设备执行的环节，以「工单创建 → 任务下发 → 结果回传」机制对接自动化模组（如镁伽流式），旨在贯通发现全链路协同，而非将既有分散流程简单迁移至线上。
