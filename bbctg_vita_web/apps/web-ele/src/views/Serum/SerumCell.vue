    ...
    <div class="three-column-layout" v-loading="loading">
      <div class="column target-column">
        <div class="column-header">
          <h3>靶点列表</h3>
          <el-tag size="small">{{ filteredTargets.length }} 个靶点</el-tag>
        </div>
      ...
      <div class="column project-column">
        <div class="column-header">
          <h3>项目进度</h3>
          <el-tag v-if="selectedTarget" size="small">{{ filteredProjects.length }} 个项目</el-tag>
        </div>
      ...
      <div class="column cell-column">
        <div class="column-header">
          ...
          <div v-if="selectedTarget" class="cell-stats">
            <el-tag v-if="overallStockStatus === 'danger'" size="small" type="danger">缺货</el-tag>
            <el-tag v-if="totalDemand > 0 && overallStockStatus !== 'success'" size="small" type="warning">需求量{{ eligibleProjects.length }}*4*{{ demandPerProject }}={{ totalDemand }}</el-tag>
            <el-tag v-if="overallStockStatus === 'success'" size="small" type="success">库存充足</el-tag>
            <el-tag size="small" type="success">{{ currentTotalStock }} 支</el-tag>
            <el-tag size="small" type="info">{{ currentCells.length }} 批次</el-tag>
          </div>
        </div>
        <div class="cell-list">
          <div v-if="!selectedTarget" class="empty-tip">
              ...
              <div class="cell-items collapsed" v-show="collapseByGenus">
                <div class="cell-item">
                  ...
                  <div v-if="cells.length > 0" class="cell-volume">
                    <el-tag size="small" type="info">{{ getUniqueCellCount(cells) }} 种</el-tag>
                  </div>
                </div>
              </div>
            </div>
